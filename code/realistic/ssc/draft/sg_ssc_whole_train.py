...
def train_model(f):
    ...
    # === Load datasets ===
    train_ds = SpikeDataset(f"/root/autodl-tmp/data_split_whole/train_f{f_str}.h5")
    val_ds   = SpikeDataset(f"/root/autodl-tmp/data_split_whole/val_f{f_str}.h5")
    test_ds  = SpikeDataset(f"/root/autodl-tmp/data_split_whole/test_f{f_str}.h5")
    ...
    torch.save(net.state_dict(), f"sg_ssc_whole_f{f_str}.pt")

# === Main ===
if __name__ == "__main__":
    #f_values = [round(0.1 * i, 1) for i in range(11)]
    f_values = [0,0.1,0.5,0.8]
    for f in f_values:
        train_model(f)
